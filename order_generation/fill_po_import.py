...
PO_import_template.xlsx template with the product data according to the specified mapping.
...
TEMPLATE_PATH = ROOT / "docs" / "PO_import_template.xlsx"
    ...
    for row_idx, data_row in enumerate(po_data, start=3):  # Start from row 3
        for col_idx, header in enumerate(headers, start=1):
            if header in data_row:
                ...
                # Only write non-empty values to avoid ERP import issues
                # Empty strings are treated as "not set" by leaving cell empty
                if value not in (None, ""):
                    ws.cell(row_idx, col_idx).value = value
    
    # Delete all empty rows after the data to prevent ERP import errors
    # Calculate the last row with data (header row 2 + number of data rows)
    last_data_row = 2 + len(po_data)
    
    # Delete all rows after the last data row
